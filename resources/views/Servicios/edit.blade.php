...
@section('contenido')
    <div class="container">
        ...
        <hr>
        <form action="{{ route('servicio.update', $servicio->id) }}" method="POST" id="upd">
            @method('PUT')
            @csrf
            <div>
                <div class="row container_galery">
            ...
            <div class="row container_galery">
                ...
                <div class="row">
                    <div class="lightbox-gallery">
                        @foreach ($servicio->imagenesServicios as $img)
                            <div>
                                <img class="rounded mx-auto d-block" src="{{ asset('imagenes/' . $img->ruta) }}"
                                    alt="{{ $img->nombre }}" />
                                <div>
                                    <form action="{{ route('servicio.eliminarImg', $img->id) }}" method="post"
                                        class="eliminar_imagen-form" id="eliminarImg">
                                        @method('DELETE')
                                        @csrf
                                        <button class="btn btn-link text-decoration-none texto-color" title="Eliminar" form="eliminarImg">
        ...
        <div>
            <div class="row container_galery">
                <button type="submit" form="upd" class="btn emp_button">Guardar</button>
            </div>
        </div>
            ...
            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('servicio.editImg', ['idServicio' => $servicio->id]) }}" method="post"
                            enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" value="" id="nom" name="nom">
                            <input type="hidden" value="" id="descript" name="descript">
                            <input type="hidden" value="" id="cost" name="cost">
                            <div class="mb-3">
                                <input class="form-control" type="file" name="archivoServicio[]" id="archivoServicio"
                                    multiple accept="image/*">
                            </div>
                            <div class="d-grid gap-2 col-6 mx-auto">
                                <button type="submit" class="btn emp_button" onclick="valores_Servicios()">Aceptar</button>
                            </div>
                        </form>
                    </div>
                </div>
...
@endsection
<script>
    function valores_Servicios() {
        var nom = document.querySelector('#nombre').value;
        var descpt = document.querySelector('#descripcion').value;
        var cos = document.querySelector('#costo').value;

        document.querySelector('#nom').value = "" + nom;
        document.querySelector('#descript').value = "" + descpt;
        document.querySelector('#cost').value = "" + cos;
    }
</script>
